...
*The Nth triangle of the series shown above. The output will consist of 32 rows and 63 columns, and will be composed of ones (1) and underscores as in the triangles above.
  */
class SierpinskiTriangles extends HackerRankExecutable {

  val board = mutable.Buffer.fill(32, 63)('_')

  def drawTriangles(n: Int) = {
    val x = board.head.size / 2
    val y = board.size
    triangle((x, 0), y, n)
    printBoard(board)
  }

  def triangle(start: (Int, Int), height: Int, n: Int) = {
    (0 until height).foreach(y => {
      board(start._2 + y)(start._1) = '1'
      (1 to y).foreach(x => {
        board(start._2 + y)(start._1 + x) = '1'
        board(start._2 + y)(start._1 - x) = '1'
      })
    })
    if (n > 0) reverseTriangle((start._1, height / 2), height / 2, n - 1)
  }

  def reverseTriangle(start: (Int, Int), height: Int, n: Int): Unit = {
    (0 until height).foreach(y => {
      board(start._2 + y)(start._1) = '_'
      (1 until height - y).foreach(x => {
        board(start._2 + y)(start._1 + x) = '_'
        board(start._2 + y)(start._1 - x) = '_'
      })
    })
    if (n > 0) {
      reverseTriangle((start._1 - height, start._2 + height / 2), height / 2, n - 1)
      reverseTriangle((start._1 + height, start._2 + height / 2), height / 2, n - 1)
      reverseTriangle((start._1, start._2 - height / 2), height / 2, n - 1)
    }
  }

  def printBoard(board: mutable.Buffer[mutable.Buffer[Char]]) = {
    board.foreach(row => {
      row.foreach({ col =>
        print(col)
      })
